...
export function attributesViewSvgFilename(baseName: string): string {
  const trimmed = baseName.trim() || 'attributes';
  const safe = trimmed.replace(/[/\\?%*:|"<>]/g, '_');
  return `${safe}.attributes.svg`;
}

export interface AttributesViewSvgLayout {
  cssW: number;
  cssH: number;
...
export function buildAttributesViewSvgString(
  p: AttributesViewSvgLayout,
): string {
